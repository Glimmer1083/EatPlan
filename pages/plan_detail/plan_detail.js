...
Page({
  ...
  onLoad:function(options){
    ...
    var that= this;
    app.editTabbar();
    var plan_index = this.getusedplan(options.plan_name);
    ...
    if (index != -1){
      this.setData({
        todayList: app.globalData.allplan[plan_index].planList[index]
      });
    }
    this.setData({
      today: today,
  ...
  getusedplan: function(plan_name){
    var index = 0;
    for (; index < app.globalData.allplan.length; index++) {
      if (app.globalData.allplan[index].name == plan_name){
        this.setData({
          plan: app.globalData.allplan[index],
          planList: app.globalData.allplan[index].planList,
          planIndex: index
        });
        break;
      }
  ...
  getdayscolor: function(date,dateList){
    var month = '';
    var days_color = [];
    for (let index = 0; index < dateList.length; index++) {
      var time_1 = new Date(dateList[index]);
      var time_2 = new Date(date);
  ...
  dayClick: function(e){
    ...
    this.getfulldate();
    // 标记
    var days_color = this.getdayscolor(selectDate,this.data.dateList);
    this.setData({
